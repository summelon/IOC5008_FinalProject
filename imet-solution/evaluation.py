...
test_transform_ablation = Compose([RandomCrop(320, pad_if_needed=True)])
...
def binarize_prediction_ablation(probabilities, threshold: float, argsorted=None,
                        min_labels=1, max_labels=10):
    """ Return matrix of 0/1 predictions, same shape as probabilities.
    """
    assert probabilities.shape[1] == N_CLASSES
...
def predict(model, loader, use_cuda: bool, half=False):
    ...
    all_outputs, all_ids = [], []
    with torch.no_grad():
        for inputs, ids in tqdm.tqdm(loader):
            inputs = inputs.cuda()
...
loader = DataLoader(
        ...
        #dataset=NormalDataset(test_root, test_df, test_transform_ablation),
        shuffle=False,
...
for i in range(1):
    df = test(model, loader, Path(f'model_efficient_b4_5'), multi=False)
    gc.collect()
    dfs.append(df)
df = pd.concat(dfs)
df = mean_df(df)
...
#df[:] = binarize_prediction_ablation(pred, threshold=args['threshold'], max_labels=args['max_labels'])
